...
## Adding sets
Add a set by using the [add set command](/getting-started/developer-experience.html#add-page-builder-article-set). Alternatively you could edit `resources/fieldsets/article.yaml` to add sets (preferably imports) to the article. In `resources/views/page_builder/_article.antlers.html` you can see the sets being loaded. Antlers will look in the `resources/views/components/` folder for partials with the handle of your set.
